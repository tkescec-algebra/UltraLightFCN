...
class SOTAStage1Config:
    ...
    # Output
    out_root: str = "seg_sota/stage1_dev80"
    summary_csv: str = "sota_stage1_results.csv"
    winners_json: str = "stage1_winners.json"
...
def _build_loaders(cfg: SOTAStage1Config, batch_size: int, *, seed: int) -> Tuple[DataLoader, DataLoader]:
...
@torch.no_grad()
def _eval_val(model: torch.nn.Module, loader: DataLoader, cfg: SOTAStage1Config) -> Dict[str, float]:
    model.eval()
    soft_sum = 0.0
    hard_sum = 0.0
    n = 0
...
def run_one(
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        for images, masks in tqdm(
            train_loader,
            desc=f"[SOTA-Stage1] {model_name}/{regime_name} seed={seed} ep={epoch}",
            leave=False,
        ):
            images = images.to(cfg.device, non_blocking=True)
            masks = masks.to(cfg.device, non_blocking=True)
...
def main() -> None:
    ...
    save_csv_rows(os.path.join(cfg.out_root, "sota_stage1_aggregate.csv"), agg_rows)
    print(f"[SOTA Stage1] Wrote: {os.path.join(cfg.out_root, cfg.summary_csv)}")
    print(f"[SOTA Stage1] Wrote: {os.path.join(cfg.out_root, 'sota_stage1_aggregate.csv')}")

    # --- Build stage1_winners.json (Phase-5 style selection: mean metric, tie-break std, then best_val_soft)
    ...
    stage1_winners = {
        ...
        "models": list(SOTA_MODELS.keys()),
        "regimes": list(SOTA_REGIMES.keys()),
    ...
    print(f"[SOTA Stage1] Wrote: {os.path.join(cfg.out_root, cfg.winners_json)}")
