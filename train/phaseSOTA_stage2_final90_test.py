...
from utils.sota_registry import SOTA_MODELS, SOTA_REGIMES, split_smp_encoder_decoder_params


@dataclass(frozen=True)
class SOTAStage2Config:
    ...
    out_root: str = "seg_sota/stage2_final90_test"
    seed_csv: str = "phaseSOTA_seed_runs.csv"
    report_json: str = "phaseSOTA_test_report.json"
...
def _build_train90_loader(cfg: SOTAStage2Config, batch_size: int, *, seed: int) -> DataLoader:
...
def _build_test_loader(cfg: SOTAStage2Config, batch_size: int) -> DataLoader:
...
@torch.no_grad()
def eval_test_failfast(model: torch.nn.Module, loader: DataLoader, cfg: SOTAStage2Config) -> Dict[str, float]:
    model.eval()
    n = 0
...
def train_one(cfg: SOTAStage2Config, phase5_winner: Dict[str, Any], model_name: str, regime_name: str, seed: int) -> Dict[str, Any]:
    ...
    model_cfg = dict(SOTA_MODELS[model_name])
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        for images, masks in tqdm(train_loader, desc=f"[SOTA-Stage2] {model_name}/{regime_name} seed={seed} ep={epoch}", leave=False):
            images = images.to(cfg.device, non_blocking=True)
            masks = masks.to(cfg.device, non_blocking=True)
...
def main() -> None:
    cfg = SOTAStage2Config()
    os.makedirs(cfg.out_root, exist_ok=True)

    phase5_winner = _read_json(cfg.phase5_winner_json)

    seed_rows: List[Dict[str, Any]] = []
    for model_name in SOTA_MODELS.keys():
        for regime_name in SOTA_REGIMES.keys():
    ...
    for model_name in SOTA_MODELS.keys():
        for regime_name in SOTA_REGIMES.keys():
            ok = [r for r in seed_rows if r["model_name"] == model_name and r["regime"] == regime_name and r["status"] == "ok"]
    ...
    print(f"[SOTA Stage2] Wrote: {os.path.join(cfg.out_root, cfg.seed_csv)}")
    print(f"[SOTA Stage2] Wrote: {os.path.join(cfg.out_root, cfg.report_json)}")
