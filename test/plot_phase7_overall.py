"""
Phase-7 desktop paper-ready plots (TEST locked; reporting only).
...
def parse_model_parts(model_id: str) -> Tuple[str, str]:
    ...
    text = str(model_id)
    if text.startswith("ultralight_phase6"):
        return ("ultralight_phase6", "")
    parts = text.split("::")
    base = parts[0] if len(parts) >= 1 else text
    regime = ""
...
def get_param_table(tr: pd.DataFrame, flops_col: Optional[str]) -> pd.DataFrame:
    source = tr.copy()
...
def aggregate_bds(
    ...
    tr_params = get_param_table(tr, flops_col)
    ...
    bds_agg = comp.groupby(["base_model", "ft_regime"], as_index=False).agg(**agg_spec)
...
def build_context(settings: PlotSettings) -> PlotContext:
    ...
    q_overall, bds_agg, flops_col, tr_params = aggregate_bds(q, ta, tr, settings.model_order)
...
def save_figure4_desktop_main(ctx: PlotContext) -> None:
    ...
    save_fig(fig, ctx.settings.outdir / "figure4_desktop_benchmark", use_tight_layout=False)
