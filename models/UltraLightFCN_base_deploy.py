...
class WindowedShiftedSelfAttention(nn.Module):
    ...
    def _build_attn_mask(
        ...
        """
        Build the shifted-window attention mask so deploy inference matches
        the base model logic. This version keeps the manual attention path
        but restores the same cyclic mask shift used in base.py.
        Shape: (nW, 1, L, L), where True means disallow attention.
        """
        ws = int(self.window_size)
        ss = int(self.shift_size)
        ...
        # 3x3 regions (TorchScript-friendly explicit indexing)
        img_mask[:, :, 0:Hp - ws, 0:Wp - ws] = cnt
        cnt += 1
        img_mask[:, :, 0:Hp - ws, Wp - ws:Wp - ss] = cnt
        cnt += 1
        img_mask[:, :, 0:Hp - ws, Wp - ss:Wp] = cnt
        cnt += 1

        img_mask[:, :, Hp - ws:Hp - ss, 0:Wp - ws] = cnt
        cnt += 1
        img_mask[:, :, Hp - ws:Hp - ss, Wp - ws:Wp - ss] = cnt
        cnt += 1
        img_mask[:, :, Hp - ws:Hp - ss, Wp - ss:Wp] = cnt
        cnt += 1

        img_mask[:, :, Hp - ss:Hp, 0:Wp - ws] = cnt
        cnt += 1
        img_mask[:, :, Hp - ss:Hp, Wp - ws:Wp - ss] = cnt
        cnt += 1
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # --- Scaled dot-product attention (TorchScript/PyTorch 1.10 friendly fallback) ---
        # --- Scaled dot-product attention (TorchScript/PyTorch 1.10 friendly fallback) ---
